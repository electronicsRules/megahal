...
sub ready {
    ...
    if ($self->should_pipeline) {
        ...
        $self->debug_printf("READY cannot-run queue=%d idle=%s", scalar @$queue, $self->is_idle ? "Y" : "N");
    }
}
...
sub request {
    ...
    my $on_read = sub {
        ...
        my $connection_close = lc($header->header("Connection") || ($self->{can_pipeline} ? "keep-alive" : "close")) eq "close";
    ...
    $self->write(join($CRLF, @headers) . $CRLF . $CRLF . $req->content);
