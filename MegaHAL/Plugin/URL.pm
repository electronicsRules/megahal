                ...
                given($url) {
                    when (m`^(?:[a-zA-Z0-9-_]+\.deviantart\.com/(?:art/[^ ]+)|(?:[^# ]+#/d[^ ]+))|(?:fav\.me/[^ ]+)$`) {
                        if ($self->{'chans'}->{$chan}->{'deviantart'}) {
                            cache_http('http://backend.deviantart.com/oembed?url='.$url,undef,60*60*12,sub {
                                my ($data)=@_;
                                ...
                                #}
                                $serv->msg($chan,(sprintf ("%s%s${C}2${B}%s${O} by ${C}9${B}%s${O} [${C}6%s${O}]",$prefix,
                                    $favme,$o->{'title'},$o->{'author_name'},$o->{'category'}
                                )));
                                return 1;
                            });
                        }
                    ...
                    when (m`^(?:www\.)?fimfiction.net\/story\/(\d+)`) {
                        if ($self->{'chans'}->{$chan}->{'fimfic'}) {
                            cache_http('http://fimfiction.net/api/story.php?story='.$1,"fimfic:$1",60*30,sub {
                                my ($data)=@_;
                                return 0 if not $data;
                                my $dat;
                                ...
                                chop $fcl;
                                chop $sfcl;
                                my $oa=sprintf ("%s${C}2${B}%s${O} by ${C}9${B}%s${O} [${C}6%s;%s;%sc;%sw;%sv${O}] [${C}6${B}%s${O}] [${C}3+%s${C}4-%s${O}] [%s] ",$prefix,
                                    $s->{'title'},$s->{'author'}->{'name'},
                                    $s->{'content_rating_text'},$s->{'status'},metric($s->{'chapter_count'}),metric($s->{'words'}),metric($s->{'total_views'}),
                                    time2str('%H%MGMT %d%b%y',$s->{'date_modified'}),
                                    metric($s->{'likes'}),metric($s->{'dislikes'}),
                                    $sfcl);
                    ...
                    when (m`^derpiboo(?:(?:\.ru)|(?:ru\.org))/(\d+)(?:\?.*)?$`) {
                        if ($self->{'chans'}->{$chan}->{'derpibooru'} && $1) {
                            cache_http('http://derpiboo.ru/'.$1.'.json',"derpibooru:$1",60*60,sub {
                                my ($data)=@_;
                                return 0 if not $data;
                                my $o;
                                eval {$o=decode_json($data);};
                                ...
                                $serv->msg($chan,(sprintf ("%s#%s (%s x %s) by ${C}9${B}%s${O} [${C}3+%s${C}4-%s${O}] [${C}12%s${O}]",$prefix,
                                    $o->{'id_number'},$o->{'width'},$o->{'height'},$o->{'uploader'},$o->{'upvotes'},$o->{'downvotes'},$o->{'tags'}
                                )));
                                return 1;
                            });
                        }
                    ...
                    when (m`^(?:www\.)?youtu(?:(?:be\.com)|(?:\.be))/`) {
                        if ($self->{'chans'}->{$chan}->{'youtube'}) {
                            ...
                            my $id=$1;
                            cache_http('http://gdata.youtube.com/feeds/api/videos/'.$id.'?v=2',"youtube:$id",60*60,sub {
                                my ($data)=@_;
                                ...
                                my $oa=sprintf "%s${C}2${B}%s${O} by ${C}9${B}%s${O} [%s] [%s] [${C}3+%s${C}4-%s${O},%sc,%sv,%sf]%s ",$prefix,
                                    $title, $author, $duration, $cat, $upv, $downv, $ncomm, $views, $favs, ($cban ? " ${C}5$cban${O} " : '');
                                my $mlen=841-15;
                                if (length($sdesc) >= ($mlen-5)-length($oa)) {
                                    $sdesc=substr($sdesc,0,($mlen-5)-length($oa)).'[...]';
                                }
