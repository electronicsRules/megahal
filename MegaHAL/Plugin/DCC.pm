...
sub new {
    ...
    $serv->reg_cb(dcc_close => sub {
        my ($this,$id,$type,$reason)=@_;
        delete $self->{'i'}->{$id};
    });
    $serv->reg_cb('iConsoleCommand' => sub {
