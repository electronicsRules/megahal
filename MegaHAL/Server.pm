...
sub connect {
    ...
    $self->{'con'}->reg_cb(disconnect => sub {
        $self->{'status'}=0;
        $self->{'timer'}=[];
        $self->{'session'}={};
        if ($_[1] ne $self->{'dc'} and not ($self->{'expect_dc'} and $_[1]=~/^EOF from server/)) {
            ...
            if ($self->{'reconnect'} >= 0) {
                push @{$self->{'timer'}}, AnyEvent->timer(after => $self->{'reconnect'}, cb => sub {
    ...
    $self->{'con'}->reg_cb(registered => sub {
        $self->{'status'}=3;
        $_[0]->unreg_me;
        push @{$self->{'timer'}},AnyEvent->timer(after => 0.5,interval => 0.5,cb => sub {
            $self->call_hook('tick');
        ...
        if ($self->{'auth'} && $self->{'authpw'}) {
            given($self->{'auth'}) {
                when('nickserv') {
                    my $grd;
                    my $tmr;
                    $grd=$self->{'con'}->reg_cb(privatemsg => sub {
                        my ($this,$nick,$ircmsg)=@_;
                        my $command=$ircmsg->{'command'};
                        my $message=$ircmsg->{'params'}->[1];
                    ...
                    $tmr=AnyEvent->timer(after => 5, cb => sub {
        ...
        if ($self->{'ping'} > 0) {
            $self->{'con'}->enable_ping($self->{'ping'},sub {
    ...
    $self->{'con'}->reg_cb(quit => sub {
        my ($this,$nick,$msg)=@_;
        delete $self->{'session'}->{$nick};
    });
    $self->{'con'}->reg_cb(kick => sub {
    ...
    $self->{'con'}->reg_cb(nick_change => sub {
...
sub common_chans {
    ...
    return scalar(map {grep {$_ eq $nick} keys %$_} values %chans);
}
...
sub disconnect {
    ...
    if ((not $fast) and ($self->registered())) {
        ...
        push @{$self->{'timer'}}, AnyEvent->timer(after => 2, cb => sub {
